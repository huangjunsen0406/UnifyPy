...
- 引擎化与插件化：用 `Engine + EventBus + PluginManager` 替代旧的 `UnifyPyBuilder`，各阶段通过事件驱动。
- 生命周期：`on_start → handle_rollback_commands → load_config → environment_check → prepare → build_executable → generate_installers → on_success → on_exit`。
- 仅支持 64 位架构：`x64 / arm64`（Linux 按 glibc/musl 生成 internal_key 与标签）。
- 打包格式范围：Windows EXE、macOS DMG、Linux DEB/RPM（已移除 MSI/AppImage/ZIP 等）。

### 3.2 配置系统 (`unifypy/core/`)
...
### 3.4 PyInstaller集成 (`unifypy/pyinstaller/`)
