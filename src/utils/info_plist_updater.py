...
# -*- coding: utf-8 -*-
"""
macOS Info.plist 更新器
在 PyInstaller 构建完成后，更新 Info.plist 添加权限描述
"""
...
class InfoPlistUpdater:
    ...
    def update_app_info_plist(self, app_path: Path, macos_config: Dict[str, Any]) -> bool:
        """
    ...
    def _update_plist_permission(self, plist_path: Path, key: str, description: str) -> bool:
        ...
        try:
            ...
            if result.returncode != 0:
                ...
                result = subprocess.run(set_cmd, capture_output=True, text=True, check=False)
    ...
    def _add_special_permissions(self, plist_path: Path, macos_config: Dict[str, Any]):
        ...
        # 添加后台音频权限（如果需要音频访问）
        if macos_config.get('audio_usage_description') or macos_config.get('microphone_usage_description'):
            try:
