...
class ParallelBuilder:
        ...
        for format_type, future in tasks:
            try:
                ...
                self.progress.on_error(
                    Exception(f"{format_type} 格式打包超时"),
                    "并行构建"
                )
                results[format_type] = False
            except Exception as e:
                self.progress.on_error(
                    Exception(f"{format_type} 格式打包失败: {e}"),
                    "并行构建"
                )
                results[format_type] = False
    ...
    def _build_single_format(
        ...
        try:
            ...
            self.progress.on_error(
                Exception(f"线程中构建 {format_type} 失败: {e}"),
                "并行构建"
            )
            return False
    ...
    def _can_use_incremental_build(self, project_dir: Path) -> bool:
        ...
        try:
            ...
            latest_build_time = max(f.stat().st_mtime for f in build_files if f.is_file())
