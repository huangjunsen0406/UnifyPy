...
# -*- coding: utf-8 -*-
"""
Linux AppImage 打包器 创建便携式AppImage格式应用.
"""
...
class AppImagePackager(BasePackager):
    ...
    def package(self, format_type: str, source_path: Path, output_path: Path) -> bool:
        ...
        if not appimagetool_path:
            self.progress.on_error(
                Exception("无法获取appimagetool工具"), "Linux AppImage打包"
            )
            return False
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # 构建AppImage
            success = self._build_appimage(
                appdir_path, output_path, appimagetool_path, appimage_config
            )

            return success

    def _create_appdir(
        self, source_path: Path, appdir_path: Path, config: Dict[str, Any]
    ) -> bool:
        """
        创建AppDir目录结构.
        """
        try:
            ...
            self.progress.on_error(
                Exception(f"创建AppDir失败: {e}"), "Linux AppImage打包"
            )
            return False
...
"""
        ...
        # 写入桌面文件
        desktop_file = (
            appdir_path / "usr" / "share" / "applications" / f"{app_name}.desktop"
        )
    ...
    def _copy_icon(self, appdir_path: Path, config: Dict[str, Any]):
        ...
        if icon_path and os.path.exists(icon_path):
            ...
            # 复制到pixmaps目录
            icon_dest = (
                appdir_path / "usr" / "share" / "pixmaps" / f"{app_name}{icon_ext}"
            )
...
"""
    ...
    def _build_appimage(
        ...
        success = self.runner.run_command(
            command, "Linux AppImage打包", "正在生成AppImage文件...", 60, shell=False
        )
        ...
        if success and output_path.exists():
            ...
            self.progress.on_error(
                Exception(f"AppImage文件生成失败: {output_path}"), "Linux AppImage打包"
            )
            return False
