...
# -*- coding: utf-8 -*-
"""
Linux Tarball 打包器 创建tar.gz格式的应用分发包.
"""
    ...
    def package(self, format_type: str, source_path: Path, output_path: Path) -> bool:
        ...
        if not self.can_package_format(format_type):
            self.progress.on_error(
                Exception(f"不支持的格式: {format_type}"), "Linux Tarball打包"
            )
            return False
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                package_dir = (
                    Path(temp_dir)
                    / f"{self.config.get('name', 'myapp')}-{self.config.get('version', '1.0.0')}"
                )
            ...
            self.progress.on_error(
                Exception(f"Tarball打包失败: {e}"), "Linux Tarball打包"
            )
            return False

    def _create_package_structure(
        self, source_path: Path, package_dir: Path, config: Dict[str, Any]
    ):
        """
        创建包目录结构.
        """
...
"""
            ...
            if isinstance(extra_file, str):
                ...
                if os.path.exists(extra_file):
                    if os.path.isfile(extra_file):
                        ...
                        shutil.copytree(
                            extra_file, package_dir / os.path.basename(extra_file)
                        )
    ...
    def _create_tarball(
        self, package_dir: Path, output_path: Path, config: Dict[str, Any]
    ) -> bool:
        """
        创建tar.gz文件.
        """
        ...
        try:
            ...
            with tarfile.open(
                output_path, "w:gz", compresslevel=compression_level
            ) as tar:
                # 获取包目录名
            ...
            if output_path.exists() and output_path.stat().st_size > 0:
                ...
                self.progress.on_error(
                    Exception(f"tar.gz文件生成失败: {output_path}"), "Linux Tarball打包"
                )
                return False

        except Exception as e:
            self.progress.on_error(
                Exception(f"创建tar.gz文件失败: {e}"), "Linux Tarball打包"
            )
            return False
