...
class MSIPackager(BasePackager):
    ...
    def package(self, format_type: str, source_path: Path, output_path: Path) -> bool:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if success and output_path.exists():
                ...
                self.progress.on_error(
                    Exception(f"MSI文件生成失败: {output_path}"),
                    "Windows MSI打包"
                )
                return False
    ...
    def _check_wix_tools(self) -> bool:
        ...
        try:
            import shutil
            return shutil.which("candle.exe") is not None and shutil.which("light.exe") is not None
        except:
            return False
    
    def _build_wxs_file(self, config: Dict[str, Any], source_path: Path, output_path: Path) -> str:
        """构建WiX源文件"""
