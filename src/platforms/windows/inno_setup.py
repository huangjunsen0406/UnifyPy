        ...
        try:
            ...
            if success:
                ...
                if output_path.exists():
                    ...
                    self.progress.on_error(
                        Exception(f"输出文件不存在: {output_path}"),
                        "Windows打包"
                    )
                    return False
            else:
...
'''
    ...
    def _build_iss_script(self, config: Dict[str, Any], source_path: Path, output_path: Path) -> str:
        """构建Inno Setup脚本"""
        ...
        if source_path.is_file():
            # 单文件模式
            source_files = f'Source: "{source_path}"; DestDir: "{{app}}"; Flags: ignoreversion'
        else:
            # 目录模式
