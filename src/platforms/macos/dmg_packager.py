...
class DMGPackager(BasePackager):
    ...
    def package(self, format_type: str, source_path: Path, output_path: Path) -> bool:
        ...
        if not app_path and source_path.is_file():
            ...
            self.progress.on_error(
                Exception(f"无法找到 .app bundle，DMG 打包需要 .app 文件：{source_path}"),
                "macOS DMG打包"
            )
            return False
        
        # 构建create-dmg命令
        command = self._build_create_dmg_command(dmg_config, app_path, output_path, create_dmg_path)
        ...
        if success and output_path.exists():
            ...
            self.progress.on_error(
                Exception(f"DMG文件生成失败: {output_path}"),
                "macOS DMG打包"
            )
            return False
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                for size in sizes:
                    ...
                    cmd = ['sips', '-z', str(size), str(size), icon_path, '--out', str(resized_icon)]
                    self.runner.run_command(cmd, "图标转换", f"生成{size}x{size}图标", 5, shell=False)
                
                # 转换为icns
                cmd = ['iconutil', '-c', 'icns', str(iconset_dir), '-o', str(output_path)]
                self.runner.run_command(cmd, "图标转换", "生成icns文件", 10, shell=False)
                
        except Exception as e:
            self.progress.warning(f"图标转换失败: {e}")
    
    def _build_create_dmg_command(self, config: Dict[str, Any], app_path: Path, output_path: Path, create_dmg_path: str) -> List[str]:
        """构建create-dmg命令"""
        ...
        if 'window_size' in config:
            ...
            if isinstance(window_size, list) and len(window_size) == 2:
                command.extend(['--window-size', str(window_size[0]), str(window_size[1])])
        ...
        if 'icon_position' in config:
            ...
            if isinstance(icon_pos, list) and len(icon_pos) == 2:
                command.extend(['--icon', app_path.name, str(icon_pos[0]), str(icon_pos[1])])
