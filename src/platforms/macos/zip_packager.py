        ...
        try:
            ...
            with zipfile.ZipFile(output_path, 'w', zipfile.ZIP_DEFLATED) as zipf:
                ...
                if source_path.is_file():
                    # 单个文件
                    self.progress.update_stage("macOS ZIP打包", 10, f"添加文件: {source_path.name}")
            ...
            if output_path.exists() and output_path.stat().st_size > 0:
                ...
                self.progress.on_error(
                    Exception(f"ZIP文件生成失败: {output_path}"),
                    "macOS ZIP打包"
                )
                return False
    ...
    def _add_extra_files(self, zipf: zipfile.ZipFile, config: Dict[str, Any]):
        ...
        for extra_file in extra_files:
            if isinstance(extra_file, str):
                ...
                target = extra_file.get('target', os.path.basename(source)) if source else None
