...
"""
macOS PKG 打包器
使用pkgbuild和productbuild创建PKG安装包
"""
...
class PKGPackager(BasePackager):
    ...
    def package(self, format_type: str, source_path: Path, output_path: Path) -> bool:
        ...
        if source_path.is_file():
            ...
            dmg_packager = DMGPackager(self.progress, self.runner, self.tool_manager, self.config)
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            pkgbuild_cmd = [
                ...
                '--identifier', pkg_config.get('identifier', f'com.example.{self.config.get("name", "MyApp")}'),
            ...
            if pkg_config.get('use_distribution', True):
                # 创建分发定义文件
                distribution_xml = self._create_distribution_xml(pkg_config, component_pkg)
            ...
            if success and output_path.exists():
                ...
                self.progress.on_error(
                    Exception(f"PKG文件生成失败: {output_path}"),
                    "macOS PKG打包"
                )
                return False
    
    def _create_distribution_xml(self, config: Dict[str, Any], component_pkg: Path) -> str:
        """创建分发定义XML文件"""
